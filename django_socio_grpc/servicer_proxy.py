        ...
        if self.grpc_async:
            ...
            async def async_handler(request, context):
                try:
                    ...
                    await sync_to_async(grpc_request_started.send)(sender=async_handler, request=request, context=context)

                    context.add_done_callback(test)
                    ...
                    response = await instance_action(
                    ...
                    # INFO - AM - 30/06/2021 - Need this in production environnement to avoid SSL end of files errors when too much connection on database
                    # await sync_to_async(grpc_request_finished.send)(sender=async_handler)
                    print("ciicicciicicic async")
                    pass
